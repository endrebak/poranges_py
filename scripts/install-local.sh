...
usage() {
  cat <<'EOF'
Usage: scripts/install-local.sh [--debug] [--skip-tests] [--current-python | --python PYTHON]
...
echo "Building and installing polaranges_py with maturin develop ${build_mode}"
if [[ -n "${build_mode}" ]]; then
  "${install_python}" -m maturin develop "${build_mode}"
else
  "${install_python}" -m maturin develop
fi
